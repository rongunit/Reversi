...
public class MenuHandler {
    ...
    public Game processMainMenu() {
        ...
        return new Game(side, getDifficulty(), mode); //default side = 'b' - black
    }
    ...
    private int getOption(int numOfOptions) {
        ...
        while (option < 1 || option > numOfOptions) {
            System.out.print("\nIncorrect input! Please input a number between 1 and " + numOfOptions + ":");
            if (scanner.hasNextInt()) {
                option = scanner.nextInt();
            } else {
                scanner.next();
    ...
    /**
     * Prints out each option from an array to the user, and asks him to choose one
     * @param options the array of options presented to player
     */
