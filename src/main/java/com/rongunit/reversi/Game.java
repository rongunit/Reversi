...
public class Game {
    ...
    Scanner scanner;
    private final int mode; // 1 - PvBot, 2 - PvP
    ...
    /***
     * Manages the game until it is over - either the board is filled or
     * one of players has no checkers left, meaning they lost
     * @param mode 1 for PvBot, 2 for PvP; gets passed by the processGame() method
     */
     ...
     * Then calls the makeMove() method with the player-chosen cell
     */
    private void processPlayerMove() {
        boolean moveExists = isMovePossible();
        if (!moveExists){
    ...
    /***
     * Changes the board state following a legal move
     * Making a move includes placing a new checker and coloring every enemy's in between the new one and a different same-colored one
